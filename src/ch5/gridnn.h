//
// Created by kangyu on 2023/12/27.
//

#ifndef SLAM_IN_AUTONOMOUS_DRIVING_GRIDNN_H
#define SLAM_IN_AUTONOMOUS_DRIVING_GRIDNN_H
...
public:
  ...
  explicit GridNN(float resolution = 0.1,
    ...
    if (dim == 2 && nearby_type_ == NearbyType::NEARBY6) {
      ...
      LOG(INFO)
          << "3D grid does not support nearby4/8 , using nearby6 instead.";
...
// 实现
template <int dim> bool GridNN<dim>::SetPointCloud(sad::CloudPtr cloud) {
...
template <> void GridNN<2>::GenerateNearbyGrids() {
  if (nearby_type_ == NearbyType::CENTER) {
    ...
    nearby_grids_ = {Vec2i(0, 0),  Vec2i(-1, 0), Vec2i(1, 0),
                     Vec2i(0, 1),  Vec2i(0, -1), Vec2i(-1, -1),
...
template <int dim>
bool GridNN<dim>::GetClosestPoint(const sad::PointType &pt,
                                  sad::PointType &closest_pt, size_t &idx) {
  // 在pt栅格周边寻找最近邻
...
bool GridNN<dim>::GetClosestPointForCloud(
    sad::CloudPtr ref, sad::CloudPtr query,
    std::vector<std::pair<size_t, size_t>> &matches) {
  matches.clear();
...
#endif // SLAM_IN_AUTONOMOUS_DRIVING_GRIDNN_H
