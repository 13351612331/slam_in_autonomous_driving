//
// Created by kangyu on 2023/12/19.
//
...
void GenerateRangeImage(PointCloudType::Ptr cloud) {
  ...
  for (const auto &pt : cloud->points) {
    ...
    int y =
        int((elevation + FLAGS_elevation_range) / ele_resolution + 0.5); // 行
    ...
    cv::imwrite("./range_image_1.png", image);
    cv::imwrite("./range_image_2.png", image_flipped);
  }
}
