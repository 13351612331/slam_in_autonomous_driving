//
// Created by kangyu on 2024/1/19.
//
#include "octo_tree.h"
#include <execution>

namespace sad {
bool OctoTree::BuildTree(const sad::CloudPtr &cloud) {
  if (cloud->empty()) {
    return false;
  }
...
void OctoTree::Insert(const sad::IndexVec &points, sad::OctoTreeNode *node) {
...
void OctoTree::ExpandNode(sad::OctoTreeNode *node,
                          const sad::IndexVec &parent_idx,
...
bool OctoTree::GetClosestPoint(const sad::PointType &pt,
                               std::vector<int> &closest_idx, int k) {
  if (k > size_) {
    LOG(ERROR) << "cannot set k larger than clous size: " << k << ", " << size_;
    return false;
  }
...
bool OctoTree::GetClosestPointMT(
    const sad::CloudPtr &cloud, std::vector<std::pair<size_t, size_t>> &matches,
    int k) {
  k_ = k;
...
void OctoTree::Knn(const Vec3f &pt, sad::OctoTreeNode *node,
...
void OctoTree::ComputeDisForLeaf(
    const Vec3f &pt, sad::OctoTreeNode *node,
    std::priority_queue<NodeAndDistanceOcto> &knn_result) const {
  ...
  if (knn_result.size() < k_) {
    ...
    // result等于k，比较current与max_dis_iter之间的差异
