//
// Created by kangyu on 2024/1/13.
//
#include "kdtree.h"
#include <execution>

namespace sad {
bool KdTree::BuildTree(const sad::CloudPtr &cloud) {
  if (cloud->empty()) {
    return false;
  }
...
void KdTree::Insert(const sad::IndexVec &points, sad::KdTreeNode *node) {
  ...
  const auto create_if_not_empty = [this](KdTreeNode *&new_node,
...
bool KdTree::GetClosestPoint(const sad::PointType &pt,
...
bool KdTree::GetClosestPointMT(const sad::CloudPtr &cloud,
...
void KdTree::Knn(const Vec3f &pt, sad::KdTreeNode *node,
...
bool KdTree::NeedExpand(
    const Vec3f &pt, sad::KdTreeNode *node,
    std::priority_queue<NodeAndDistance> &knn_result) const {
...
bool KdTree::FindSplitAxisAndThresh(const sad::IndexVec &point_idx, int &axis,
                                    float &th, sad::IndexVec &left,
                                    sad::IndexVec &right) {
