...
namespace sad {
IMUPreintegration::IMUPreintegration(sad::IMUPreintegration::Options options) {
...
void IMUPreintegration::Integrate(const sad::IMU &imu, double dt) {
  // 去掉零偏的测量
  Vec3d gyr = imu.gyro_ - bg_; // 陀螺
  Vec3d acc = imu.acce_ - ba_; // 加速度计

  // 更新dv , dp , 见(4.13)(4.16)
  dp_ = dp_ + dv_ * dt + 0.5f * dR_.matrix() * acc * dt * dt;
  dv_ = dv_ + dR_ * acc * dt;

  // dR先不更新，因为A，B矩阵需要现在的dR
  ...
  dP_dbg_ = dP_dbg_ + dV_dbg_ * dt -
            0.5f * dR_.matrix() * acc_hat * dR_dbg_;         // (4.39e)
...
NavStated IMUPreintegration::Predict(const sad::NavStated &start,
  ...
  Vec3d pj =
      start.R_ * dp_ + start.p_ + start.v_ * dt_ + 0.5 * grav * dt_ * dt_;
