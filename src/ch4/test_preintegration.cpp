//
// Created by kangyu on 2023/11/1.
//
#include "eskf.hpp"
...
TEST(PREINTEGRATION_TEST, ROTATION_TEST) {
  ...
  Vec3d constant_omega(0, 0, M_PI); // 角速度为180度/s，转1s应该等于转180度
  Vec3d gravity(0, 0, -9.8); // Z向上， 重力方向为负
  ...
  for (int i = 1; i <= 100; i++) {
    ...
    auto this_status = pre_integ.Predict(start_status, gravity);
...
TEST(PREINTEGRATION, ACCELERATION_TEST) {
  // 测试在恒定加速度运行下的预积分情况
  double imu_time_span = 0.01;    // IMU测量间隔
  Vec3d gravity(0, 0, -9.8);      // Z向上， 重力方向为负
  Vec3d constant_acce(0.1, 0, 0); // x方向上的恒定加速度
  ...
  io.SetIMUProcessFunc([&](const sad::IMU &imu) {
      ...
      .SetGNSSProcessFunc([&](const sad::GNSS &gnss) {
        ...
        if (last_state_set && last_gnss_set) {
          ...
          LOG(INFO) << "state before eskf update: " << state_bef_update.p_
                    << std::endl;
          LOG(INFO) << "state after eskf update: " << update_state.p_
                    << std::endl;

          auto state_pred = preinteg->Predict(last_state, eskf.GetGravity());
          LOG(INFO) << "state in pred: " << state_pred.p_ << std::endl;
...
void Optimize(sad::NavStated &last_state, sad::NavStated &this_state,
  ...
  v0_bg->setId(2);
  v0_vel->setEstimate(last_state.bg_);
  ...
  LOG(INFO) << "inertial init err: " << edge_inertial->chi2() << std::endl;
  ...
  LOG(INFO) << "inertial bg rw: " << edge_gyro_rw->chi2() << std::endl;
  ...
  LOG(INFO) << "inertial ba rw: " << edge_acc_rw->chi2() << std::endl;
  ...
  LOG(INFO) << "gnss0 init err: " << edge_gnss0->chi2() << std::endl;
  ...
  LOG(INFO) << "gnss1 init err: " << edge_gnss1->chi2() << std::endl;
  ...
  sad::NavStated corr_state(this_state.timestamp_, v1_pose->estimate().so3(),
                            ...
                            v1_ba->estimate());
  // LOG(INFO) << "corr state in opt: " << corr_state << std::endl;

  // 获取结果，统计各类误差
  LOG(INFO) << "chi2/error: " << std::endl;
