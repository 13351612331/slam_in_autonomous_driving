...
bool StaticIMUInit::AddOdom(const sad::Odom &odom) {
  // 判断车辆是否静止
  if (init_success_) {
    return true;
  }
...
bool StaticIMUInit::TryInit() {
  ...
  if (cov_gyro_.norm() > options_.max_static_gyro_var) {
    std::cerr << "陀螺仪测量噪声太大" << cov_gyro_.norm() << " > "
              << options_.max_static_gyro_var << std::endl;
    return false;
  }

  if (cov_acce_.norm() > options_.max_static_acce_var) {
    std::cerr << "加速度计测量噪声太大" << cov_acce_.norm() << " > "
              << options_.max_static_acce_var << std::endl;
    return false;
  }
  ...
  std::cout << "IMU 初始化成功，初始化时间= "
            ...
            << ", norm: " << gravity_.norm();
  std::cout << "mean gyro: " << mean_gyro.transpose()
            << " acce: " << mean_acce.transpose() << std::endl;
