//
// Created by kangyu on 2023/9/5.
//

#ifndef SLAM_IN_AUTONOMOUS_DRIVING_ESKF_HPP
#define SLAM_IN_AUTONOMOUS_DRIVING_ESKF_HPP
...
public:
  ...
  struct Options {
    ...
    // NOTE IMU噪声项都为离散时间， 不需要再乘以dt，可以由初始化器指定IMU噪声
...
template <typename S> bool ESKF<S>::Predict(const sad::IMU &imu) {
  assert(imu.timestamp_ >= current_time_);

  double dt = imu.timestamp_ - current_time_;
  if (dt > (5 * options_.imu_dt_) || dt < 0) {
    // 时间间隔不对，可能是第一个IMU数据，没有历史信息
    std::cout << "[eskf.hpp::Predict][INFO]: skip this imu because dt_ = " << dt
              << std::endl;
  ...
  F.template block<3, 3>(6, 6) =
      SO3::exp(-(imu.gyro_ - bg_) * dt).matrix();         // theta对theta
  F.template block<3, 3>(6, 9) = -Mat3T::Identity() * dt; // theta对bg

  // mean and cov prediction
  // 这行其实没必要算，dx_在重置之后应该为零，因此这步可以跳过，但F需要参与Cov部分计算，所以保留
  dx_ = F * dx_;
...
template <typename S> bool ESKF<S>::ObserveGps(const sad::GNSS &gnss) {
  // GNSS 观测修正
...
#endif // SLAM_IN_AUTONOMOUS_DRIVING_ESKF_HPP
